...
%forlpf
fl=100;                   
ff=[0 0.01 0.02 1]; fa=[1 1 0 0];   %should be large than abs bw , well below than 2fc
h=firpm(fl,ff,fa);
% figure(11),freqz(h)

%costasloop
theta1=zeros(1,length(t)); theta1(1)=0;   %initialize estimate
theta2=zeros(1,length(t)); theta2(1)=0;  

zs1=zeros( 1 , fl+1); zc1=zeros (1 , fl +1);  % buffer for lpfs.. (state   vector for 
zs2=zeros( 1 , fl+1); zc2=zeros (1 , fl +1);  %time domain filtering with zeros intialization


for k=1:length(t)-1        %zs contain past fl+1 inputs
    zs1=[zs1(2:fl+1), 2*r(k)*sin(2*pi*fo*t(k)+ theta1(k))] ;  %In phase channel
    zc1=[zc1(2:fl+1), 2*r(k)*cos(2*pi*fo*t(k)+ theta1(k))] ;  % phi channel..quadrature null effect
    lpfs1=fliplr(h)*zs1' ;  lpfc1=fliplr(h)*zc1';               %new output of filters  %%time domain filtering
    ...
    carest(k)= cos(2*pi*fo*t(k)+theta2(k)+theta1(k)); %huhhhh
...
figure(23)
periodogram(matchedfiltersignal);
...
tnow        = srrclen*M+1;     %current time ...because of srrcdelay 
tau=0;
...
% return
%% decision directed equalizer as ps
% xs is downsampled signal 
taps_eq = 7;
...
for i  = taps_eq+1:length(xs)
    ...
    e  = quantalph(b_eq'*rr,[-3,-1,1,3])-b_eq'*rr;   %desired..output- equalizer output   
...
y=conv(flip(header),quantized);   %change ..and see...why
